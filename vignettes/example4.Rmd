...
Examples files containing `1kg_phase1_chr22.bed`, `1kg_phase1_chr22.bim`, `1kg_phase1_chr22.fam`, and `1kg_phase1_chr22.vcf` files and a corresponding gene list in `glist-hg19` can be downloaded via the `SEAGLE.zip` file from [http://jocelynchi.com/SEAGLE/](http://jocelynchi.com/SEAGLE/).  To follow along with the PLINK conversion procedures, you will first need to install the PLINK1.9 from [https://www.cog-genomics.org/plink/1.9/](https://www.cog-genomics.org/plink/1.9/) software and unzip the example `SEAGLE.zip` file.  Afterwards, you can type the PLINK commands below in a command prompt or terminal window in the folder where these files are located. This will produce .raw files that you can read into R to obtain the relevant genetic marker matrix ${\bf G}$ for input into `SEAGLE`.

## Preparing GWAS Data with PLINK1.9

For GWAS data in the .bed + .bim + .fam format, we recommend first converting to the .raw format with PLINK1.9.  To illustrate chromosome-wide gene-based analysis, we employ all the genes in chr22 as an example.  We type the following commands in command prompt or terminal to produce a .raw file for each gene.  Each .raw file includes all SNPs for the corresponding gene.
